...
if is_debug():
    $execute unless data storage coc:energy networks[].bubbles[{uuid: "$(bubble_uuid)"}] run \
...
$data modify storage coc:energy networks[].bubbles[{uuid: $(bubble_uuid)}] set from storage coc:temp bubble

path = ./calculate_capacity
raw f"$function {path} {{bubble_uuid: \"$(bubble_uuid)\"}}"