as @e[type=armor_stand,tag=coc.offering_altar,dx=0] function ./as_altar:
    scoreboard players set $suc coc.dummy 0
    if data storage coc:temp Item unless entity @s[tag=coc.has_item]:
        summon item ~.5 ~1 ~.5 {Item:{id:"minecraft:stone",Count:1b,tag:{}},PickupDelay:32767s,Tags:["coc.offering_item"]}
